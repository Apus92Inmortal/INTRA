...
create table if not exists public.shipment_declarations (
  id uuid primary key default gen_random_uuid(),
  ...
  created_at timestamptz not null default now(),
  constraint shipment_declarations_shipment_user_key unique (shipment_id, user_id)
);

alter table public.shipment_declarations enable row level security;

drop policy if exists "Users can view own shipment declarations" on public.shipment_declarations;
create policy "Users can view own shipment declarations"
  on public.shipment_declarations
  for select
  to authenticated
  using (auth.uid() = user_id);

drop policy if exists "Users can insert own shipment declarations" on public.shipment_declarations;
create policy "Users can insert own shipment declarations"
  on public.shipment_declarations
  for insert
  to authenticated
  with check (auth.uid() = user_id);
...
create unique index if not exists shipments_tracking_code_key
...
grant execute on function public.create_shipment_with_payment_draft(uuid, uuid, text, text, numeric, numeric, boolean, text) to authenticated;
