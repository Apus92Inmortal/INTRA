...
export default async function AdminDisputesPage() {
  ...
  try {
    ...
    const [{ data: disputeRows, error: disputeError }, { data: reportRows, error: reportError }] = await Promise.all([
      admin
        .from("payments")
        .select(
    ...
    if (disputeError || reportError) {
      loadError = disputeError?.message ?? reportError?.message ?? "No pudimos cargar los casos administrativos."
    } else {
      ...
      const { data: matchRows, error: matchError } = matchIds.length
        ? await admin
            .from("matches")
            .select("id, shipment_id, trip_id, status, disputed_at, resolved_at, resolution_notes")
            .in("id", matchIds)
      ...
      if (matchError) {
        ...
        if (shipmentError || tripError) {
          loadError = shipmentError?.message ?? tripError?.message ?? "No pudimos cargar los envíos relacionados."
        } else {
          ...
          if (profileError) {
            ...
            disputes = disputePayments.map((payment) => {
              ...
              return {
                ...
                reporterName: fullName(shipment?.owner_id, "Cliente sin nombre"),
                ...
                reason: payment.dispute_reason || "Disputa abierta desde la app",
                state: getDisputeAdminState(payment),
                createdAt: payment.dispute_opened_at,
                resolvedAt: payment.dispute_resolved_at ?? match?.resolved_at ?? null,
                paymentStatus: payment.status,
                resolutionAction:
                  typeof metadata.admin_dispute_resolution === "string" ? metadata.admin_dispute_resolution : null,
                resolutionNotes:
            ...
            alerts = reportEvents.map((report) => {
              const match = report.match_id ? matches.get(report.match_id) ?? null : null
              ...
              return {
                ...
                trackingCode: shipment?.tracking_code ?? null,
                reporterName: fullName(report.reported_by, "Usuario sin nombre"),
    ...
    loadError = error instanceof Error ? error.message : "No pudimos cargar el módulo de disputas."
  }

  if (loadError || !hasAccess) {
    return (
      <section className="rounded-3xl border border-intra-danger-border bg-intra-card p-6 shadow-sm sm:p-8">
        <h2 className="text-2xl font-bold text-intra-blue">Disputas</h2>
